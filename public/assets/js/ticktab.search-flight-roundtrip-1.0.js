...
$(window).load(function(){
    ...
    $('.return-result #slider-range-price').slider({
        range: true,
        min: 50000,
        max: 50000000,
        values: [50000, 50000000],
        slide: function(event, ui){
            $('.return-result #amount-price').val('IDR ' + ui.values[ 0 ].toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1.') + ' - ' + ui.values[ 1 ].toString().replace(/(\d)(?=(\d\d\d)+(?!\d))/g, '$1.'));
        }
    });
    $('.return-result #amount-price').val('IDR ' + $('.return-result #slider-range-price').slider('values', 0) +  ' - ' + $('.return-result #slider-range-price').slider('values', 1));
    ...
    function cursorNavReturn(){
        if($('.return-result .scheduling ul li.before-last').is(':visible')){
            $('.return-result .scheduling a.nav-sche.prev-sche').fadeOut('fast');
        }else{
            $('.return-result .scheduling a.nav-sche.prev-sche').fadeIn('fast');
        }

        if($('.return-result .scheduling ul li.after-last').is(':visible')){
            $('.return-result .scheduling a.nav-sche.next-sche').fadeOut('fast');
        }else{
            $('.return-result .scheduling a.nav-sche.next-sche').fadeIn('fast');
        }
    }
    
    $('.return-result .scheduling a.nav-sche').click(function(event){
        event.preventDefault();
        
        if(buttonInctive == false){
            buttonInctive = true;
            
            var thisSelecting = $('.return-result .scheduling ul li.showing.selecting');
            $('.return-result .scheduling ul li.showing').removeClass('selecting');

            if($(this).hasClass('prev-sche')){
                $('.return-result .scheduling ul li.showing').last().animate({'width': 'toggle'}).removeClass('showing').addClass('hide-after');
                $('.return-result .scheduling ul li.hide-before').last().animate({'width': 'toggle'}, function(){
                    buttonInctive = false;
                    cursorNavReturn();
                }).removeClass('hide-before').addClass('showing');
                thisSelecting.prev().addClass('selecting');
            }else{
                $('.return-result .scheduling ul li.showing').first().animate({'width': 'toggle'}).removeClass('showing').addClass('hide-before');
                $('.return-result .scheduling ul li.hide-after').first().animate({'width': 'toggle'}, function(){
                    buttonInctive = false;
                    cursorNavReturn();
                }).removeClass('hide-after').addClass('showing');
                thisSelecting.next().addClass('selecting');
    ...
    $('.return-result ul.the-result li .pricing a').click(function(event){
        event.preventDefault();
